...
#define __RAFUNCTION__ "process_file"
	...
	while (token = strtok(NULL, space)) {
		  ...
		  if (! (token = strtok(NULL, space))) {
		     ERROR( "Error reading block data\n");
			 return;
	      }
	      ...
	      switch(readahead_errno) {
	              case 0: 
	            	  if(flag_debug)
	        	      ERROR("%s:%s:%d:Loaded %s %d %d\n",__FILE__,__RAFUNCTION__,__LINE__,filename, block_start, block_end);
	        	      break;
	              case EBADF:
	            	  if(flag_debug)
	            	  ERROR("%s:%s:%d:Bad file: %s\n",__FILE__,__RAFUNCTION__,__LINE__,filename);
	              case EINVAL:
	            	  if(flag_debug)
	            	  ERROR("%s:%s:%d:Invalid filetype for readhead: %s\n",__FILE__,__RAFUNCTION__,__LINE__,filename);
	        	      break;
	      }
...
#define __RAFUNCTION__ "process_files"
	...
	if(flag_debug) {
		ERROR("%s:%s:%d:Attempting to load list: %s\n",__FILE__,__RAFUNCTION__,__LINE__,filename);
	}
	...
	if (fd<0) {
		if(flag_debug) {
			ERROR("%s:%s:%d:failed to open list: %s\n",__FILE__,__RAFUNCTION__,__LINE__,filename);
		}
		return;
	}
	...
	if (fstat(fd, &statbuf)<0) {
		if(flag_debug) {
			ERROR("%s:%s:%d:failed to fstat list: %s\n",__FILE__,__RAFUNCTION__,__LINE__,filename);
		}
		return;
	}
	...
	if (!file || file == MAP_FAILED) {
		if(flag_debug) {
			ERROR("%s:%s:%d:failed to mmap list: %s\n",__FILE__,__RAFUNCTION__,__LINE__,filename);
		}
		return;
	}
	if(flag_verbose) 
	...
	while (iter) {
		...
		if (next) {
			...
			if((next - iter) >= MAXLINE) {
				ERROR("%s:%s:%d:item in list too long =%s!\n",__FILE__,__RAFUNCTION__,__LINE__,next);
			} else if (next-iter > 1) {
