...
static int digestblock()
{
	...
	pstfilestat = (STFILE_STAT *) calloc(ifilecount,sizeof(STFILE_STAT));

	if (pstfilestat == NULL) {
		ERROR("pstfilestat malloc failed\n");
		return -1;
	}
//    memset(pstfilestat,0,ifilecount*sizeof(STFILE_STAT));
...
static int gettracebuffersize(unsigned int *pbuffertotal)
{
	char value[PROPERTY_VALUE_MAX] = { '\0' };
	if (property_get(PLATFORM_TYPE, value, NULL)) {
		if ((strcmp(value, ZYGOTE64_32) == 0) || (strcmp(value, ZYGOTE32_64) == 0)) {
			*pbuffertotal = TRACE_NEED_BUFFER_SIZE_DOUBLE_ARCH;

		} else if ((strcmp(value, ZYGOTE32) == 0) || (strcmp(value, ZYGOTE64) == 0)) {
			*pbuffertotal = TRACE_NEED_BUFFER_SIZE_SINGLE_ARCH;
		}
...
int trace(char* psystemdev,char* pdatadev) {
	...
	unsigned int uitrace_buffer_total=0;
	...
	if (set_value(idfd, DO_OPEN_EXEC_ENABLE, TRUE,
		...
		if(gettracebuffersize(&uitrace_buffer_total)==0)
		{
			ERROR("error tracing_buffer size\n");
			goto error;
		}

		INFO(" tracing_buffer size ==%d\n",uitrace_buffer_total);

     /*set one cpu buffer_size_kb*/
	if (set_value(idfd, TRACE_BUFFER, uitrace_buffer_total/icpucount, NULL) < 0)
		{
